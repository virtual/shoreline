<div class="d-flex">
  <form class="siteSearch" role="search" method="GET" action="/search/SearchRedir.aspx">
    <div class="input-group main-search">
      <span class="input-group-prepend">
        <button tabindex="-1" class="input-group-text bg-transparent" name="SubmitSearch" type="submit">
          <span class="fa fa-search"></span>
          <span class="sr-only">Search</span>
        </button>
      </span>
      <label for="search1"><span class="sr-only">Enter search terms</span>
        <input id="search1" type="search" name="EnterSearch" class="entersearch form-control border-left-0 border"
          placeholder="Search Shoreline" value="" autocomplete="on"></label>
    </div>
  </form>
  <button id="searchcancel1" class="btn btn-link ml-3" data-toggle="collapse" data-target="#searchmenusub"
    aria-controls="searchmenusub" aria-expanded="true">Cancel</button>
</div>